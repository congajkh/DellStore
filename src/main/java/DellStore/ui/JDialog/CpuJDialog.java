...
public class CpuJDialog extends javax.swing.JDialog {
    ...
    private void btn_themActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_themActionPerformed
        ...
        String tocDo = txt_tocdo.getText().trim(); // không validate
        String loai = txt_loai.getText().trim();
        int trangThai = rdo_conhang.isSelected() ? 1 : 0;

// 1️⃣ Kiểm tra bắt buộc
        if (ten.isEmpty() || loai.isEmpty() || tocDo.isEmpty()) {
            JOptionPane.showMessageDialog(this,
                    "Vui lòng nhập đầy đủ tên và loại CPU",
                    "Lỗi",
                    JOptionPane.ERROR_MESSAGE);
            return;
        }

// 2️⃣ Tạo object CPU và thêm vào DB
        Cpu newCPU = new Cpu();
        newCPU.setTen(ten);
        ...
        cpuDAOImpl dao = new cpuDAOImpl();
        dao.insert(newCPU); // insert vào DB

// 3️⃣ Thông báo và cập nhật bảng
...
// Lấy object CPU theo dòng được chọn
...
// 1️⃣ Kiểm tra bắt buộc
        ...
        int confirm = JOptionPane.showConfirmDialog(this,
                "Bạn có chắc muốn cập nhật CPU này không?",
                "Xác nhận",
...
// 2️⃣ Cập nhật thông tin vào object
        cpuUpdate.setTen(ten);
        cpuUpdate.setToc_do(tocDo);
        cpuUpdate.setLoai(loai);
        cpuUpdate.setTrang_thai(trangThai);

// 3️⃣ Gọi DAO để cập nhật
        cpuDAOImpl dao = new cpuDAOImpl();
        dao.update(cpuUpdate);

// 4️⃣ Thông báo và reload bảng
        JOptionPane.showMessageDialog(this, "Cập nhật CPU thành công!");
        loadTableCPU();  // reload bảng sau khi sửa
        clearForm();     // xóa trắng form nếu muốn
